...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding, is3d):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            raise ValueError(f"Unsupported layer type '{char}'. MUST be one of ['g', 'r', 'c']")
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=2, basic_module=DoubleConv, conv_layer_order='gcr',
                 num_groups=8, padding=1, upscale=2, is3d=True):
        super().__init__()
        if apply_pooling:
            if is3d:
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, basic_module=DoubleConv,
                 conv_layer_order='gcr', num_groups=8, padding=1, is3d=True):
        super(Decoder, self).__init__()

        self.upsampling = InterpolateUpsampling(mode='nearest')
